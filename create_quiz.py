...
class Quiz:
    def __init__(self, title, description, time_limit):
        self.title = title
...
class QuizCreationApp:
    ...
    def setup_title_page(self, quiz_type):
        ...
        tk.Label(self.title_frame, text="Time Limit (in minutes):").pack()
        self.time_limit_entry = tk.Entry(self.title_frame, width=50)
        self.time_limit_entry.pack()

        self.no_time_limit_var = tk.BooleanVar()
        self.no_time_limit_check = tk.Checkbutton(self.title_frame, text="No Time Limit", variable=self.no_time_limit_var)
        self.no_time_limit_check.pack(pady=10)
    ...
    def setup_quiz_creation_page(self):
        ...
        if self.no_time_limit_var.get():
            ...
            try:
                ...
                messagebox.showwarning("Input Error", "Time limit must be a positive integer.")
                return

        if self.quiz_type == "MCQ":
            self.quiz = MultipleChoiceQuiz(title, description, time_limit)
        elif self.quiz_type == "True/False":
            self.quiz = TrueFalseQuiz(title, description, time_limit)
        elif self.quiz_type == "Short Answer":
            self.quiz = ShortAnswerQuiz(title, description, time_limit)
    ...
    def save_quiz(self):
        ...
        with open(filename, "w", newline="", encoding="utf-8") as file:
            ...
            if self.quiz.time_limit is None:
                writer.writerow(["Time Limit", "None"])
            else:
                writer.writerow(["Time Limit", self.quiz.time_limit])
